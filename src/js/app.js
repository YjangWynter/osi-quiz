...
var Quiz = function () {
    var self = this;
    this.init = () => self._bindEvents
    this._selectAnswer =(choice, question) => {
        //Disable all choices
        question.querySelectorAll('.quiz-choice').classList.remove("active")
        question.querySelectorAll('.quiz-choice').classList.add("disabled")
        //Activate selected choice
        choice.querySelectorAll('.quiz-choice').classList.add("disabled")
        choice.querySelectorAll('.quiz-choice').classList.remove("active")
    ...
    this._isComplete = function () {
        var answersChosen = 0;
        $("ul[data-quiz-question]").each( () => {
            if (this.querySelectorAll(".quiz-choice.active").length > 0) {
                answersChosen++;
            }
        });
    ...
    this._tallyResponses = function () {
        ...
        let elements = document.querySelectorAll("ul[data-quiz-question]");
        elements.forEach.call(elements, () => choiceList.push(this.querySelectorAll(".quiz-choice.active").dataset.data-choice-value))
        return choiceList;
    }

    this._calcResult = function () {
        /*
        This is where we will place answer matrix for agencies
        const nameOfAgency = new Array([],[],[],...)
        // agencyAnswers[0]
        ...
        const peaceCorps = new Array(["a", "b", "c", "d", "e"], ["b", "d", "e", "f"], ["b", "c", "d", "e"], ["a", "b", "f"]);
        // orgsAnswers[2] = Operation AmeriCorps
        const opAmeriCorps = new Array(["a"], ["e"], ["c"], ["a"]);
        // orgsAnswers[3] = National Health Corps
        const natHealthCorps = new Array(["a", "b", "c", "d", "e"], ["a", "b", "f"], ["d", "e"], ["b","d","f"]);
    ...
    this._showResult = function () {
        var resultBox = document.querySelectorAll(".result")
        var resultList = self._calcResult();


        resultBox.classList.add("resultComplete jumbotron");
        resultBox.innerHTML = `<h1 class='mb-4'><p>Here is your result!</p> <p>You matched with <i>${ORGINFO[resultList[0]].name}</i></p></h1><p>${ORGINFO[resultList[0]].info}</p><p class="text-center"><a class="my-3 mx-auto btn btn-primary btn-lg" href="${ORGINFO[resultList[0]].website}" target="_blank"><strong>Visit Website</strong></a></p>`
        ...
        if (resultList.length > 1) {
            resultBox.appendChild(`<hr class="my-4"><h2><strong> You also matched with:</strong></h2> <ul id="accordion">${resultToString()}</ul><br>`);
        }
    ...
    this._bindEvents = function () {
        ...
        document.querySelectorAll(".jumbotron").each( () => jumboList.push(this));
        ...
        document.querySelectorAll(".quiz-choice").addEventListener("click", function () {
            ...
            $("body, html").animate({
                scrollTop: (jumboList[parseInt(question.dataset.data-quiz-question)]).offset().top - 25
            }, 500);
        });
    }
}
